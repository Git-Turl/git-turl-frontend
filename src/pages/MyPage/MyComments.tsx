...
export function MyComments() {
  ...
  useEffect(() => {
    ...
    getMyComments({ sort: 'latest', page: 0, size: 50 })
      .then((res) => {
        if (cancelled) return;
        if (res.isSuccess && res.result?.commentList) {
          setComments(res.result.commentList);
        } else {
          setLoadError(res.message || '댓글을 불러오지 못했어요.');
        }
      })
      .catch((err) => {
        if (cancelled) return;
        console.error('[my-comments] load error', err);
    ...
    <div className="min-h-screen p-8">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex items-center gap-2 mb-8 text-sm">
          ...
          <span className="text-gray-600">내가 쓴 댓글</span>
        </div>

        <h1 className="text-3xl mb-8 text-gray-900">내가 쓴 댓글</h1>
        ...
        {comments.length > 0 && (
          <div className="mt-6 text-xs text-gray-400 flex items-center gap-1">
            <MessageCircle className="w-3 h-3" />총 {comments.length}개
          </div>
        )}
      </div>
    </div>
