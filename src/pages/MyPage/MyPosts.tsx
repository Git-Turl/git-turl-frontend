...
import { getMyBoards, type MemberBoardItem } from '../../api/member';
...
export function MyPosts() {
  ...
  useEffect(() => {
    ...
    getMyBoards({ sort: 'latest', page: 0, size: 50 })
      .then((res) => {
        if (cancelled) return;
        if (res.isSuccess && res.result?.boardList) {
          setBoards(res.result.boardList);
        } else {
          setLoadError(res.message || '게시글을 불러오지 못했어요.');
        }
      })
      .catch((err) => {
        if (cancelled) return;
  ...
  return (
    <div className="min-h-screen p-8">
      <div className="max-w-5xl mx-auto">
        ...
        <h1 className="text-3xl mb-8 text-gray-900">내가 쓴 글</h1>
        ...
        {boards.length > 0 && (
          <div className="mt-6 text-xs text-gray-400 flex items-center gap-1">
            <MessageCircle className="w-3 h-3" />총 {boards.length}개
          </div>
        )}
      </div>
    </div>
